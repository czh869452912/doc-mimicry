...
- Assistant-ui advanced capabilities that require additional product/backend semantics: BranchPicker, retry/reload semantics, attachments, dictation, assistant-ui `SelectionToolbar` replacement for the editor selection bar, and deeper tool-call history beyond the current semantic event cards.
